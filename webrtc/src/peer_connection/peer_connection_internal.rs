...
impl PeerConnectionInternal {
    ...
    pub(super) async fn start_rtp(
        ...
        if !is_renegotiation {
            self.undeclared_media_processor(&track_details);
        } else {
    ...
    fn undeclared_media_processor(self: &Arc<Self>, track_details: &[TrackDetails]) {
        let dtls_transport = Arc::clone(&self.dtls_transport);
        let is_closed = Arc::clone(&self.is_closed);
        let pci = Arc::clone(self);

        // Build a set of SSRCs declared in the initial remote SDP so the acceptor
        // loop can skip them. On renegotiation `undeclared_media_processor` is not
        // called again; newly-negotiated SSRCs will pass through to
        // `handle_incoming_ssrc`, which re-reads the current remote description.
        let ssrcs: HashSet<_> = track_details
            .iter()
            .flat_map(|d| d.ssrcs.iter().cloned())
            .collect();

        let ssrcs = Arc::new(ssrcs);

        // SRTP acceptor
        let srtp_ssrcs = Arc::clone(&ssrcs);
        tokio::spawn(async move {
            ...
            loop {
                ...
                // If the ssrc is in the SDP, it was declared and should have been handled by the
                // normal RTP processing flow, so we ignore it here
                if srtp_ssrcs.contains(&stream.get_ssrc()) {
                    continue;
                }
        ...
        {
            let dtls_transport = Arc::clone(&self.dtls_transport);
            let srtcp_ssrcs = Arc::clone(&ssrcs);
            tokio::spawn(async move {
                loop {
                    ...
                    // Skip declared SSRCs — they are handled by the normal RTCP path
                    if srtcp_ssrcs.contains(&stream.get_ssrc()) {
                        continue;
                    }
